<template>
    <section class="py-10 md:py-14 lg:py-15 bg-white scroll-mt-[210px] md:scroll-mt-[210px]" id="clients">
        <div class="container mx-auto px-4">
            <SectionTitle :label="siteSettings?.clients_label" :title="siteSettings?.clients_title"
                :icon="['fas', 'handshake']" label-variant="default" title-variant="default"
            ...
            <div class="flex flex-col items-center gap-10">
                                ...
                                <div v-for="(page, pageIndex) in paginatedClients" :key="`page-${pageIndex}`"
                                    ...
                                    <div class="flex flex-wrap justify-center gap-6 px-4">
                                        <div v-for="client in page" :key="client.id"
                                            class="flex flex-col items-center justify-center p-6 bg-white rounded-lg border-2 border-gray-200 shadow-sm hover:shadow-lg hover:border-primary transition-all duration-300 w-full sm:w-[calc(50%-1rem)] md:w-[calc(33.333%-1.5rem)] lg:w-[calc(25%-1.5rem)] max-w-sm">
                                            <picture class="w-full h-32 flex items-center justify-center mb-4">
                                                ...
                                                <img v-lazy="getLogoUrl(client)" :alt="client.client_name"
                                                    class="max-h-32 max-w-full object-contain transition-transform duration-300 hover:scale-110"
                                                    @error="handleImageError" />
                                            </picture>
                                            <span class="text-lg font-bold text-primary text-center leading-tight">
                                                {{ client.client_name }}
                                            </span>
                                        </div>
                                    </div>
                ...
                <!-- Pagination Controls -->
                <div v-if="!isLoading && clients && clients.length > 0" class="mt-6 mb-2">
